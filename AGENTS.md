...
- Check draggable and droppable windows with `scripts/dev/drag_window.sh` (launch, windows, oswindows, drag, drag-pane, snap, key, trace, shot); never drive the pointer or the keyboard with ad hoc `cliclick` calls.

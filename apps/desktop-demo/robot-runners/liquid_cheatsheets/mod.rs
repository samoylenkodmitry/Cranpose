//! Per-reference Liquid UI visual fixtures.
//!
//! Every example wrapper selects exactly one case. The robot sends synthetic
//! pointer events while fresh snapshots are taken from the visible X11 window;
//! the checked-in target sheet is never regenerated by an E2E run.

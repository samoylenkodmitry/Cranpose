...
//! 3. SlotReusePool metadata tracking works
//!
//! Run with:
//! ```bash
...
fn main() {
    ...
    AppLauncher::new()
        ...
        .with_test_driver(|robot| {
            ...
            println!("  ✓ Initial rendering works ({} items visible)", initial_items.len());
            ...
            if let Some((_, y, _, _)) = find_text_in_semantics(&robot, "Item #3") {
                ...
                if !after_scroll.is_empty() && after_scroll[0] > 0 {
                    println!("  ✓ Scroll works (first visible: Item #{})", after_scroll[0]);
                } else {
            ...
            if click_button("Set usize::MAX") {
                ...
                if find_text_in_semantics(&robot, "items").is_some() {
            ...
            if click_button("Jump to Middle") {
                ...
                if middle_items.is_empty() {
                    ...
                    for check in [9223372036854775807_usize, 9223372036854775808, 9223372036854775809] {

        ...
        .with_test_driver(|robot| {
            ...
            let check_item = |name: &str, expected_y: f32, _expected_height: f32| {
                if let Some((_, y, _, h)) = find_text_in_semantics(&robot, name) {
                    println!("Found {name} at y={y:.1}, h={h:.1}");

                    let y_diff = (y - expected_y).abs();
                    if y_diff > 1.0 {
                        println!("  BUG: {name} y={y:.1} but expected {expected_y:.1}");
                    }
                } else {
            ...
            if let Some((_, y, _, _)) = find_text_in_semantics(&robot, "Item1") {
                assert!(
                    y >= 99.0,
                    "CONFIRMED BUG: Item1 is at y={y:.1}, expected > 100.0. Measuring is broken!"
                );
                let expected = 115.2;
                if (y - expected).abs() > 2.0 {
                    println!("Warning: Item1 y={y:.1}, expected {expected:.1}");
                }
            }

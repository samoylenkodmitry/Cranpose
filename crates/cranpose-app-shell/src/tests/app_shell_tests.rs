...
#[test]
fn a_dialog_takes_focus_when_it_opens() {
    let _guard = test_guard();
    let root_key = location_key(file!(), line!(), column!());
    let mut shell = AppShell::new(TestRenderer::default(), root_key, || {
    ...
    let tree = shell.semantics_tree().expect("the dialog is in the tree");
...
fn find_semantics_with_role(
    ...
    if node.widget_role == Some(role) {
        return Some(node);
    }
    node.children
        .iter()
        .find_map(|child| find_semantics_with_role(child, role))
}
#[test]

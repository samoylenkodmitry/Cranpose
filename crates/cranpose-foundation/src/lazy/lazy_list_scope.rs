...
pub struct LazyListIntervalContent {
    ...
    total_count: usize,
    range_index: RefCell<Option<RangeKeyIndex>>,
}

/// The slot ids of one range of items mapped to their indices: the nearest
/// range around the viewport that lookups ask for, mapped once however many
/// keys a measure looks up in it and however long the list is.
struct RangeKeyIndex {
...
impl LazyListIntervalContent {
    ...
    pub fn new() -> Self {
        Self {
            intervals: Vec::new(),
            total_count: 0,
            range_index: RefCell::new(None),
        }
    }

    fn invalidate_cache(&self) {
        *self.range_index.borrow_mut() = None;
    }
    ...
    fn mapped_keys(&self) -> usize {
        self.range_index
            .borrow()
            .as_ref()
            .map_or(0, |index| index.indices.len())
    }
    ...
    pub fn get_index_by_key_in_range(
        ...
        self.get_index_by_slot_id_in_range(key.to_slot_id(), range)
    }

    /// Returns the index of an item with the given slot ID, or None if not found.
    ///
    /// Slot IDs are generated by `LazyLayoutKey::to_slot_id()`. This scans the
    /// whole list and keeps nothing: a list may hold a million items, and a map
    /// of every key would hold tens of megabytes. Lookups near the viewport
    /// belong to `get_index_by_slot_id_in_range`, which maps its range once.
    #[must_use]
    ...
    /// Returns the index of an item with the given slot ID, searching only
    /// within the range. The range's keys are mapped on the first lookup in it
    /// and kept until another range is asked for or the content changes.
    pub fn get_index_by_slot_id_in_range(
        ...
        let range = start..range.end.clamp(start, self.total_count);
        let mut cached = self.range_index.borrow_mut();
        if cached.as_ref().is_none_or(|index| index.range != range) {
            let indices = range
                .clone()
                .map(|index| (self.get_key(index).to_slot_id(), index))
                .collect();
            *cached = Some(RangeKeyIndex { range, indices });
        }
        cached
            .as_ref()
            .and_then(|index| index.indices.get(&slot_id).copied())
    }

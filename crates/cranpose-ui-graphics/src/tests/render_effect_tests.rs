...
// forwards on purpose: one fixed call site, so the test can check that
// `RuntimeShader::new` hands back the source it cached for that site.
fn runtime_shader_from_reuse_callsite(source: &str) -> RuntimeShader {
    RuntimeShader::new(source)
}

// forwards on purpose: another fixed call site, so the test can check that new
// source text through the same site replaces the source cached for it.

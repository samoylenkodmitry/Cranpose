...
/// Missed vsyncs in [`MISS_WINDOW`] that one frame queued holds through.
/// Compose's renderer, which also runs a frame ahead, misses 4 to 10% of
/// vsyncs on the benchmark's layout screens on a Pixel 9 Pro and keeps its
/// queue short; giving up one frame queued at the first miss instead left

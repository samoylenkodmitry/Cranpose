...
#[test]
fn window_config_hands_its_title_and_parts_to_the_window() {
    let config = WindowConfig::borderless("Panel", 100.0, 50.0)
        .with_focus(WindowFocus::Never)
        .on_close_requested(|| {});
    assert_eq!(config.title(), "Panel");
    let expected_options = config.options.clone();
    ...
    assert!(events.on_close_requested.is_some());
    assert!(events.on_moved.is_none());

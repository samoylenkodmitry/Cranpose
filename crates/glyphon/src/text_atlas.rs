...
impl InnerAtlas {
    ...
    /// Padding around each glyph allocation to prevent texture bleeding
    /// when using bilinear (Linear) texture sampling.
    pub(crate) const GLYPH_PADDING: u16 = 1;

    pub(crate) fn try_allocate(&mut self, width: usize, height: usize) -> Option<Allocation> {
        let padding = Self::GLYPH_PADDING as usize * 2;
        let size = size2((width + padding) as i32, (height + padding) as i32);
    ...
    pub(crate) fn grow(
        ...
        // Re-upload glyphs. The stored atlas x/y already include the padding
        // offset, so we use them directly for upload position.

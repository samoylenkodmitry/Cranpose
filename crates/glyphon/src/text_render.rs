    ...
    pub fn prepare_with_depth_and_custom<'a>(
        ...
        for text_area in text_areas {
            ...
            for glyph in text_area.custom_glyphs.iter() {
                ...
                let (exact_x, exact_y, x_bin, y_bin) = if glyph.snap_to_physical_pixel {
                    (x.round(), y.round(), SubpixelBin::Zero, SubpixelBin::Zero)
                } else {
                    let (_, x_bin) = SubpixelBin::new(x);
                    let (_, y_bin) = SubpixelBin::new(y);
                    (x, y, x_bin, y_bin)
                };
            ...
            for run in layout_runs {
                for glyph in run.glyphs.iter() {
                    let cache_key = text_cache_key(
                        glyph,
                        text_area.cache_left,
                        text_area.cache_top,
                        text_area.scale,
                        text_area.subpixel_y,
                    );
                    let (exact_x, exact_y) =
                        exact_text_position(glyph, text_area.left, text_area.top, text_area.scale);
        ...
        let vertices_raw = bytemuck::cast_slice(vertices);
...
fn exact_text_position(
    ...
    let x = (glyph.x + x_offset).mul_add(scale, left);
    let y = (glyph.y - y_offset).mul_add(scale, top);
    (x, y)
}
...
fn text_cache_key(
    ...
    let x_offset = glyph.font_size * glyph.x_offset;
    let y_offset = glyph.font_size * glyph.y_offset;
    let cache_x = (glyph.x + x_offset).mul_add(scale, cache_left);
    let exact_y = (glyph.y - y_offset).mul_add(scale, cache_top);
    let cache_y = if subpixel_y { exact_y } else { exact_y.trunc() };
    ...
    R: FnMut(RasterizeCustomGlyphRequest) -> Option<RasterizedCustomGlyph>,
{
    let details =
            ...
            .get(&metadata.cache_key)
        {
            ...
            let (gpu_cache, atlas_id, inner) = if should_rasterize {
                ...
                let pad = crate::text_atlas::InnerAtlas::GLYPH_PADDING as u32;

                // Upload glyph data at padded offset within the allocation.
                state.queue.write_texture(
                    TexelCopyTextureInfo {
                        texture: &inner.texture,
                        mip_level: 0,
                        origin: Origin3d {
                            x: atlas_min.x as u32 + pad,
                            y: atlas_min.y as u32 + pad,
                            z: 0,
                        },
                ...
                // Store atlas coordinates with padding offset so UV references
                // the actual glyph data, not the padding border.
                (
                    GpuCacheStatus::InAtlas {
                        x: atlas_min.x as u16 + crate::text_atlas::InnerAtlas::GLYPH_PADDING,
                        y: atlas_min.y as u16 + crate::text_atlas::InnerAtlas::GLYPH_PADDING,
    ...
    // Compute exact float position, applying glyph image placement offsets.
    let exact_x = metadata.x + details.left as f32;
    let exact_y =
        (metadata.line_y * metadata.scale_factor).round() + metadata.y - details.top as f32;

    // Integer position for clipping decisions (floor).
    let ix = exact_x.floor() as i32;
    let iy = exact_y.floor() as i32;
    // Fractional offset to preserve for sub-pixel vertex placement.
    let fract_x = exact_x - ix as f32;
    let fract_y = exact_y - iy as f32;
    ...
    Ok(Some(GlyphToRender {
        pos: [x as f32 + fract_x, y as f32 + fract_y],

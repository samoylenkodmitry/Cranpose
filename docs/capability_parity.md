...
| camera            | ■ `AVCaptureSession` | ■ Camera2 (`CranposeCamera`) | □ | □ | frames pushed as `CameraFrame` with the turn carried as `rotation_degrees` (`upright_rgba8` applies it in the conversion pass); observable state and lens list (`CameraLenses`, `LensFacing`); bounded latest-wins analysis stream; stills asked for rather than waited on. Desktop capture (`camera-native`, nokhwa) was removed: nokhwa 0.10.11 (latest) reaches AVFoundation through nokhwa-bindings-macos 0.2.4, which pulls a 2020-era cocoa/core-graphics/metal stack, and its `input-native` feature is not target-gated so it also pulls nokhwa-bindings-linux on every desktop target — five duplicate dependency version families (bitflags, cfg-if, core-foundation, core-foundation-sys, getrandom, the last via flume/nanorand) the duplicate-dependency budget refuses. The intended replacement is a direct `objc2-av-foundation` backend on macOS; `objc2-av-foundation` 0.3.2 is already in the dependency graph, so it adds no new families |

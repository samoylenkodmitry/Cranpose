...
| camera            | ■ `AVCaptureSession` | ■ Camera2 (`CranposeCamera`) | ● macOS only (`camera-desktop`) | □ | frames pushed as `CameraFrame` with the turn carried as `rotation_degrees` (`upright_rgba8` applies it in the conversion pass); observable state and lens list (`CameraLenses`, `LensFacing`); bounded latest-wins analysis stream; stills asked for rather than waited on. macOS and iOS share one file, `apple_camera.rs`, because AVFoundation is one stack; a Mac reports no turn, no torch and no flash, and lists the built-in camera beside continuity and USB ones instead of a phone's fixed lenses. A Mac hands frames only to a signed, bundled application carrying `NSCameraUsageDescription`. Linux and Windows have no backend: the nokhwa one that used to serve them reaches AVFoundation through nokhwa-bindings-macos 0.2.4, which pulls a 2020-era cocoa/core-graphics/metal stack, and its `input-native` feature enables the v4l2 bindings by feature rather than by target, so macOS and Windows both took the Linux chain too. That is five duplicate dependency version families (bitflags, cfg-if, core-foundation, core-foundation-sys, getrandom) the duplicate-dependency budget refuses. Windows wants a Media Foundation backend on the `windows` crate the wgpu chain already carries, and Linux a v4l2 one on the `rustix` already in the tree; either adds no new family |
...
| incoming content  | □ | ■ shares (`ACTION_SEND`, `ACTION_SEND_MULTIPLE`) and "Open with" (`ACTION_VIEW`) for the media types the declaration opens (`declare(..).opening(..)`), at launch and through `onNewIntent` | ■ files dropped on the window, and documents named in `argv` | ■ files dropped on the canvas | one `IncomingContent` stream on every platform that has one; winit owns the iOS `UIApplicationDelegate`, so `openURL` is not reachable, and a macOS `.app` opened from the Finder is told by an Apple Event winit does not surface — drops arrive, opens do not |

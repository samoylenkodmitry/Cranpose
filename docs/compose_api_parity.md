...
| The per-property `Modifier::` shorthands (`error`, `password`, `pane_title`, `traversal_index`, `heading`, `role`, `content_description`, `live_region`, `hide_from_accessibility`, `merge_descendants`, `dropdown_list`, `value_picker`, `expand`, `collapse`, `on_long_click`, `dismiss`) | -- | Cranpose's own shape | Compose has none of these as modifiers; each is a property inside `semantics { }`. They read better than a closure for one property. The rows above name the Compose property each one mirrors. |

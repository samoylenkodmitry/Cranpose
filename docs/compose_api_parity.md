...
| `Modifier.semantics { liveRegion = LiveRegionMode.Polite }` | `Modifier::semantics(\|config\| config.live_region = Some(LiveRegionMode::Polite))` | Implemented | Same two modes. accesskit and ARIA carry it natively; on iOS and Android Cranpose reads the changed text out itself, because iOS has no live region and Android reads one on a virtual view only through its host view. |

...
* [ ] **Text is the main bottleneck: remeasure shaped-buffer reuse after the architectural fixes**: the original telemetry run showed `prepared_layout_cache_hit_rate=96.7%`, `size_hit_rate=78.3%`, but `text_cache_hit_rate=0.2%`, `reshape_rate=99.8%`, `reshapes=577`, `reuses=1`. Since then, actual text nodes now reuse node-local prepared layouts across measure + render, and the shared WGPU shaped-buffer cache is a bounded LRU with occupancy/eviction telemetry. Rerun the heavy markdown/generic harness to quantify what remains, especially utility probes and text-field paths that are not yet node-local paragraphs.

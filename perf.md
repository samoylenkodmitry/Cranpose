...
* [ ] **Text is the main bottleneck: remeasure shaped-buffer reuse after the architectural fixes**: reruns after the node-local paragraph and range-based wrap changes show the problem has moved rather than vanished. The generic harness now reports `prepare_with_options_calls=2000`, `prepare_fast_path_rate=54.2%`, `prepared_layout_cache_hit_rate=46.3%`, `size_hit_rate=82.0%`, `text_cache_hit_rate=7.4%`, `text_cache_occupancy=256`, `text_cache_evictions=133`, `reshape_rate=100.0%`, `reshapes=296`, `reuses=0`, and render-side `skip_rate=0.0%`. The markdown scrollbar workload is much healthier (`prepare_fast_path_rate=99.2%`, `prepared_layout_cache_hit_rate=57.8%`, `size_hit_rate=90.6%`, `text_cache_hit_rate=0.0%`, `reshapes=3`, `reuses=0`), which suggests node-local paragraph reuse is working for actual text nodes while utility/text-field probes and renderer-side prepared batch reuse still churn.
